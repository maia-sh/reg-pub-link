...
ctgov_crossreg <- read_rds(here("data", "processed", "ctgov", "ctgov-crossreg.rds"))
drks_crossreg <- read_rds(here("data", "processed", "drks", "drks-crossreg.rds"))
# registry_crossreg <- read_rds(here("data", "processed", "registry-crossreg.rds"))
trn_reported_wide <- read_rds(here("data", "processed", "trn-reported-wide.rds"))
trn_reported_long <- read_rds(here("data", "processed", "trn-reported-long.rds"))
...
write_rds(registry_crossreg, here::here("data", "processed", "registry-crossreg.rds"))

# There are up to 4 cross-registrations
# max(registry_crossreg$n_crossreg)
...
crossreg_reg <-
  ...
  tidylog::left_join(distinct(intovalue, id, pmid, doi), by = "id")
...
write_rds(cross_registrations, here("data", "processed", "cross-registrations.rds"))
...
n_crossreg_reg <-
  ...
  select(-crossreg_trn, -crossreg_registry, -n_crossreg) %>%
...
write_rds(n_cross_registrations, here("data", "processed", "n-cross-registrations.rds"))
