# Analysis of TRN in PubMed metadata given TRN in abstract/full-text
# Function to create tables for si by registry ----------------------------

tbl_si_registry <- function(var, var_name) {
  trials %>%
    mutate(has_iv_trn_abs_or_ft = has_iv_trn_abstract | has_iv_trn_ft) %>%
    filter({{var}}) %>%
...
tbl_si_ft <- tbl_si_registry(has_iv_trn_ft, "TRN in full-text")
