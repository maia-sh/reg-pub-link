...
**Secondary analyses** We explored<!--# tested --> the relationships between (1) TRN in abstract or full-text, and TRN in PubMed metadata, as well as between (2) TRN in PubMed metadata and a linked publication in the registration. We also explored the relationship between registration-publication links and (1) registry, and (2) prospective registration <!--# not sure about this. i think if we do this then glm for the various practices -->. We looked at change in registration-publication linking practices over time. We also looked at reported cross-registrations in the registries as well as additional TRNs reported in trial publications.<!--# We also looked at reporting rates by UMCs, randomized controlled trials (RCTs), and ICMJE-status. We also explored the association of TRN reporting practices with other responsible trial research practices, namely prospective registration and summary results reporting. All analyses are exploratory. -->
...
## Automated pipeline for data sources and collection
...
<!--# Using registry APIs, when available, and web scraping, we checked whether each unique TRN resolved to a valid trial registration. currently placeholder "TRUE" such that all TRNs resolve in the registries --><!--# Since i'm focusing iv trn only, should i change this section so not ALL the trns? then would have to change my backend work. or can keep this section but maybe not report all the details in text but just appendix? -->
...
Overall, DRKS trials appear to be less robustly linked than ClinicalTrials.gov trials. Overall, we observed positive trends for TRN reporting in full-text and abstracts. <!--# add test? --> Figure \@ref(fig:plot-links-year-reg) shows the registration-publication links over time for ClinicalTrials.gov and DRKS.
